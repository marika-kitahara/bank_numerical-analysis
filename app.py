...
def patch_ak_in_original_xlsx(file_bytes: bytes, ak_results: list[float | None]) -> str:
    """後方数値データ(加工版)だけを、一時xlsxへ直接書き出す省メモリ版。

    起動時には何も追加処理をしない。
    Excel作成ボタン押下後だけ openpyxl / tempfile を読み込み、
    完成ファイル本体をBytesIOやsession_stateへ保持しない。
    """
    import os
    import tempfile
...
st.caption("AK計算後は「後方数値データ(加工版)」1シートだけを一時ファイルへ直接出力します。元60MBブック全体は再構築しません。")
...
if st.button("🛠️ AK列計算済みExcelを作成", key="create_ak_export"):
    try:
        ...
        with st.spinner("AK列を計算し、1シートだけのExcelをディスクへ作成しています…"):
...
if ak_export_path:
    ...
    if os.path.exists(ak_export_path):
        export_file = open(ak_export_path, "rb")
        st.download_button(
